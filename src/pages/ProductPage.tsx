...
export default function ProductPage() {
  ...
  useEffect(() => {
    if (product) {
      document.title = `${product.title} | CookieSoft`
    ...
    return (
      <PageSection background="white">
        <div className="max-w-6xl mx-auto text-center">
          <h1 className="text-3xl font-bold text-dark-900">Product Not Found</h1>
          <p className="mt-4 text-dark-500">The product page you requested could not be found.</p>
          <Link to="/" className="mt-6 inline-flex px-6 py-3 rounded-full bg-primary-600 text-white font-semibold hover:bg-primary-700 transition">
            Back to Home
          </Link>
        </div>
      </PageSection>
        ...
        <div className="grid gap-10 lg:grid-cols-[2fr_1fr]">
          <div className="space-y-8">
            <div className="rounded-3xl border border-dark-100 bg-primary-50/60 p-8">
              <h2 className="text-2xl font-semibold text-dark-900">What this product delivers</h2>
              <p className="mt-4 text-dark-500 leading-relaxed">{product.description}</p>
            </div>
          ...
          <aside className="space-y-6 rounded-3xl border border-dark-100 bg-white p-8 shadow-sm">
            <div>
              <h3 className="text-lg font-semibold text-dark-900">Ready to explore this product?</h3>
              <p className="mt-3 text-dark-500 leading-relaxed">{product.callToAction}</p>
            </div>
            <Link
              to="/"
